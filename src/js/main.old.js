const modelName = "../models/sakura.vrm";
...
const FPSGeometry = new THREE.PlaneGeometry(0.06, 0.03);

const FPSMaterial = new THREE.MeshBasicMaterial({
    //color: 0xff0000,
    transparent: true,
    opacity: 0.8,
...
window.FPSMesh = new THREE.Mesh(FPSGeometry, FPSMaterial);
...
function animate() {
    ...
    if (currentVrm) {
        ...
        FPSMaterial.map = new THREE.CanvasTexture(FPSCanVas); // 更新紋理
        FPSMaterial.map.needsUpdate = true;
    }
...
loader.load(
    ...
    gltf => {
        ...
        THREE.VRM.from(gltf).then(vrm => {
            ...
            const faceBone = currentVrm.humanoid.getBoneNode(THREE.VRMSchema.HumanoidBoneName.Head);
            //const facePosition = new THREE.Vector3();
            //facePosition.setFromMatrixPosition(faceBone.matrixWorld);
            FPSMesh.position.set(0.035, 0.02, -0.11);
            FPSMesh.rotation.y = Math.PI * 300 / 360;

            // 添加Mesh對象到VRM模型的臉頰上
            faceBone.add(FPSMesh);
        });
    },
